...
class EncoderLayer(torch.nn.Module):
    ...
    def forward(self, x, mask):
        # 计算自注意力,维度不变
        # [b, 50, 32] -> [b, 50, 32]
        score = self.mh(x, x, x, mask)  # Encoder 中QKV都是 encoder input提供
...
class DecoderLayer(torch.nn.Module):
    ...
    def forward(self, x, y, mask_pad_x, mask_tril_y):
        # 先计算y的自注意力,维度不变
        # [b, 50, 32] -> [b, 50, 32]
        y = self.mh1(y, y, y, mask_tril_y)  # self-attention只需要下三角矩阵即可，掩盖掉需要预测的部分

        # 结合x和y的注意力计算,维度不变
        # [b, 50, 32],[b, 50, 32] -> [b, 50, 32]
        y = self.mh2(y, x, x, mask_pad_x)  # Q由Decoder提供，KV由Encoder提供，cross attention的时候需要知道Encoderinput的pad信息
